package com.aqtx.app.location.activity;
...
import com.aqtx.app.location.adapter.IconListAdapter;
import com.aqtx.app.location.helper.MapHelper;
import com.aqtx.app.location.helper.NimLocationManager;
import com.aqtx.app.location.model.NimLocation;
...
public class NavigationAmapActivity extends UI implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(com.aqtx.app.R.layout.map_view_amap_navigation_layout);
		mapView = (MapView) findViewById(com.aqtx.app.R.id.autonavi_mapView);
	...
	private void initView() {
		sendButton = findView(com.aqtx.app.R.id.action_bar_right_clickable_textview);
		sendButton.setText(com.aqtx.app.R.string.location_navigate);
		sendButton.setOnClickListener(this);
		sendButton.setVisibility(View.INVISIBLE);

		myLocationFormatText = getString(com.aqtx.app.R.string.format_mylocation);
	}
	...
	private void initLocation() {
		...
		if (TextUtils.isEmpty(desAddressInfo)) {
			desAddressInfo = getString(com.aqtx.app.R.string.location_address_unkown);
		}
	...
	private void updateSendStatus() {
		...
		if (TextUtils.isEmpty(myAddressInfo)) {
			setTitle(com.aqtx.app.R.string.location_loading);
			sendButton.setVisibility(View.GONE);
		} else {
			setTitle(com.aqtx.app.R.string.location_map);
			sendButton.setVisibility(View.GONE);
		}
	}
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case com.aqtx.app.R.id.action_bar_right_clickable_textview:
			navigate();
			break;
		}
	}
	...
	public void onLocationChanged(NimLocation location) {
		if (location != null && location.hasCoordinates()) {
			if (firstLocation) {
				...
				// 缩放到可见区
				int boundPadding = getResources().getDimensionPixelSize(com.aqtx.app.R.dimen.friend_map_bound_padding);
	...
	private void showLocationFailTip() {
		if (firstLocation && firstTipLocation) {
			...
			myAddressInfo = getString(com.aqtx.app.R.string.location_address_unkown);
			Toast.makeText(this, com.aqtx.app.R.string.location_address_fail, Toast.LENGTH_LONG).show();
		}
	}
	...
	private MarkerOptions defaultMarkerOptions() {
		...
		markerOptions.icon(BitmapDescriptorFactory.fromResource(com.aqtx.app.R.drawable.pin));
	...
	private void doNavigate(final NimLocation origin, final NimLocation des) {
		...
		if (infos.size() >= 1) {
			...
			dialog.setTitle(getString(com.aqtx.app.R.string.tools_selected));
			dialog.show();
		} else {
			IconListAdapter.IconListItem item = new IconListAdapter.IconListItem(getString(com.aqtx.app.R.string.friends_map_navigation_web), null, null);
			items.add(item);
			CustomAlertDialog dialog = new CustomAlertDialog(this, items.size());
			dialog.setAdapter(adapter, new DialogInterface.OnClickListener() {
				@Override
	...
	private View getMarkerInfoView(Marker pmarker) {
		...
		View view = getLayoutInflater().inflate(com.aqtx.app.R.layout.amap_marker_window_info, null);
		TextView textView = (TextView) view.findViewById(com.aqtx.app.R.id.title);
