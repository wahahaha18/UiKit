...
import com.aqtx.app.main.activity.WelcomeActivity;
...
public class AVChatNotification {
    ...
    private void buildCallingNotification() {
        if (callingNotification == null) {
            ...
            String tickerText = String.format(context.getString(com.aqtx.app.R.string.avchat_notification), displayName);
            int iconId = com.aqtx.app.R.drawable.ic_stat_notify_msg;

            PendingIntent pendingIntent = PendingIntent.getActivity(context, CALLING_NOTIFY_ID, localIntent, PendingIntent
                    .FLAG_UPDATE_CURRENT);
            callingNotification = makeNotification(pendingIntent, context.getString(com.aqtx.app.R.string.avchat_call), tickerText, tickerText,
                    iconId, false, false);
        }
    }
    ...
    private void buildMissCallNotification() {
        if (missCallNotification == null) {
            ...
            String title = context.getString(com.aqtx.app.R.string.avchat_no_pickup_call);
            String tickerText = NimUserInfoCache.getInstance().getUserDisplayName(account) + ": 【网络通话】";
            int iconId = com.aqtx.app.R.drawable.avchat_no_pickup;
