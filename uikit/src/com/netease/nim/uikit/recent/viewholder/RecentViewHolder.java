...
public abstract class RecentViewHolder extends TViewHolder implements OnClickListener {
    ...
    // FIXME: 2016/12/28 最近联系人显示草稿
    private void updateMsgLabel() {
        // 显示消息具体内容
        String text = "";
        String content = UserPreferences.getSharedPreferences().getString(recent.getContactId(), "");
        if (!TextUtils.isEmpty(content)) {
            text = "[草稿]" + content;
        } else {
            text = getContent();
        }
        MoonUtil.identifyFaceExpressionAndTags(context, tvMessage, text, ImageSpan.ALIGN_BOTTOM, 0.45f);
